...
define(function() {
	...
	var self = {
		 ...
		 * @example
		 * ```js
		 * sfd.utils.copyValues(srcData, destData, [
		 *   'email',
		 *   'name'
		 * ]);
		 * // => 아래처럼 세팅하는 것과 동일
		 ...
		 * ```
		 */
		copyValues: function(src, dest, keys, option) {
			option = option || {};
		...
		formatPeriod: function(fromDate, toDate, options) {
			...
			result = result == options.tilt ? '-' : result.trim();
		...
		weekAge: function(dateOfBirth, baseDate) {
			...
			// 출산예정일까지 남은 일수
			var remainigDays = sfd.utils.daysBetween(baseDate, dateOfBirth);
			return Math.max(Math.floor((280 - remainigDays) / 7), 4);
		},
		...
		/**
		 * 이메일 주소를 사용자이름, 도메인으로 분리
		 * @category 변환
		 * @param {String} email 이메일 주소
		 * @return {Array} 사용자이름, 도메인 분리한 배열. email 인자가 없는 경우 각각 빈문자열로 설정됨.
		 * @example
		 * ```js
		 * var emailComponents = sfd.utils.parseEmail('abcd@mail.com');
		 * // => ['abcd', 'email.com']
		 * 
		 * sfd.utils.parseEmail(''); // => ['', '']
		 * ```
		 */
